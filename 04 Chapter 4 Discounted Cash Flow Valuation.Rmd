...
$$
\begin{aligned}
\mathbf{PV}_{\text{annuity}} &= C \cdot \left[ \frac{1 - (1 + r)^{-T}}{r}  \right]
\end{aligned}
$$

```{r problem 12 define pvia}
pvia <- function(rate, term, growth=0) {
  (1 - ((1 + growth) / (1 + rate))^(term)) / (rate - growth)
}
```

```{r problem 12 define present value of annuity}
...
```{r solution 12}
(investment.x <- pv.annuity(4500, .05, 9))
(investment.y <- pv.annuity(7000, .05, 5))
investment.x > investment.y
cat("\n")
(investment.x <- pv.annuity(4500, .22, 9))
(investment.y <- pv.annuity(7000, .22, 5))
investment.x > investment.y
```
...
An investment offers \$4,900 per year for 15 years, with the first payment occurring one year from now. If the required return is 8 percent, what is the value of hte investment? What would the value be if the payments occurred for 40 years? For 75 years? Forever?
...
$$
\begin{aligned}
\mathbf{PV}_{\text{annuity}} &= C_0 \left( \frac{1 - \left( \frac{1 + g}{1 + r} \right)^T}{r - g}  \right) \\ \\
\mathbf{C}_{0} &= 65,000 \times .05 \\
  &= 3,250.00 \\ \\
\mathbf{PV}_{\text{annuity}} &=  3,250.00 \left( \frac{1 - \left( \frac{1 + .04}{1 + .1} \right)^{40}}{.1 - .4} \right) \\ \\
  &= 48,420.76
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{PV} &= C \times \frac{1 - (1 + r)^{-T}}{r} \\ \\
\mathbf{PV}_{\text{10\%}} &= 6,800 \times \frac{1 - (1 + .1)^{-15}}{.1} \\
  &= 51,721.34 \\ \\
\mathbf{PV}_{\text{5\%}} &= 6,800 \times \frac{1 - (1 + .05)^{-15}}{.05} \\
  &= 70,581.67 \\ \\
\mathbf{PV}_{\text{15\%}} &= 6,800 \times \frac{1 - (1 + .15)^{-15}}{.15} \\
  &= 39,762.12
\end{aligned}
$$

There is a non-linear inversely proportional relationship between the interest rate and the value of an annuity. As the interest rate increases the value of the annuity decreases; and as the interest rate decreases the value of the annuity increases. A 5% drop in the interest rate means a \$18,860.33 increase in the value of the annuity; and a 5% increase in interest rates means a \$11,959.22 drop in the value in the annuity.
...
$$
\begin{aligned}
\mathbf{FV}_{\text{annuity}} &= \text{C} \frac{(1 + \frac{r}{m})^{mT} - 1}{\frac{r}{m}} \\ \\
t &= \frac{ \ln{\left( \frac{\frac{r}{m} \times \text{FV}}{C} + 1  \right)}  }{\ln{(1 + \frac{r}{m})}} \\
  &= 73.038
\end{aligned}
$$

We round up to 74 months.
...
with $r$ as the monthly rate $\text{APR} = r \times 12$.

$$
\begin{aligned}
\mathbf{PV} &= C \times \frac{1 - (1 + r)^{-T}}{r} \\ \\
\frac{65,000}{1,320} &= \frac{1 - (1 + r)^{-60}}{r} \\ \\
0 &= \frac{1 - (1 + r)^{-60}}{r} - 49.24242
\end{aligned}
$$
...
(root <- uniroot(f, lower=1e-5, upper=1, tol=1e-8)$root)

1320 * (1 - (1+root)^(-60))/(root)
```

The maximum APR is 0.67%.

---

## Calculating loan payments

You need a 30-year, fixed-rate mortgage to buy a new home for \$250,000. Your mortgage bank will lend you the money at a 5.3 percent APR for this 360-month loan. However, you can only afford monthly payments of \$950, so you offer to pay off any remaining loan balance at the end of the loan in the form of a single balloon payment. How large will this balloon payment have to be for you to keep your monthly payment at \$950?

Calculate the future value of the loan in 30 years. This is a 30 year loan of monthly payments thus $n = 30 \times 12 = 360$. The rate is $r = .053/12 = .004417$

$$
\begin{aligned}
\mathbf{FV}_{\text{loan}} &= P \times (1 + r)^{n} \\
\mathbf{FV}_{\text{loan}} &= 250,000 \cdot (1 + .004417)^{360} \\
  &= 1,221,799
\end{aligned}
$$

The monthly payments can be considered an annuity lasting 30 years, with a monthly coupon of \$950.

$$
\begin{aligned}
\mathbf{FV}_{\text{payments}} &= \text{PMT} \times \left[ \frac{(1 + r)^{n} - 1}{r}  \right] \\ \\
\mathbf{FV}_{\text{payments}} &= 950 \times \left[ \frac{(1 + .004417)^{360} - 1}{.004417} \right] \\ \\
  &= 836,050.60
\end{aligned}
$$

The monthly payments will be \$950 a month for the 30 years. The amount of the loan remaining to be paid of (the balloon payment at the end):

$$
\begin{aligned}
\mathbf{Balloon} &= \text{FV}_{\text{loan}} - \text{FV}_{\text{payments}} \\
\mathbf{Balloon} &= 1,221,799 - 836,050.60 \\
  &= 385,748.40
\end{aligned}
$$

The balloon payment will be \$385,748.40.

---

## Present and future values

The present value of the following cash flow stream is \$7,300 when discounted at 8 percent annually. What is the value of the missing cash flow?

```{r}
year <- c(1,2,3,4)
cash.flow <- c(1500, '?', 2700, 2900)
knitr::kable(data.frame(year, cash.flow))
```

$$
\begin{aligned}
\mathbf{PV} &= \frac{C_{1}}{1 + r} + \frac{C_{2}}{(1 + r)^{2}} + \frac{C_{3}}{(1 + r)^3} + \frac{C_{4}}{(1 + r)^4} \\ \\
7,300 &= \frac{1,500}{1 + .08} + \frac{C_{2}}{(1 + .08)^{2}} + \frac{2,700}{(1 + .08)^3} + \frac{2,900}{(1 + .08)^4} \\ \\
7,300 &= 1388.89 + \frac{C_{2}}{1.1664} + 2,143.35 + 2,131.59 \\ \\
1636.17 &= \frac{C_{2}}{1.1664} \\ \\
\mathbf{C_{2}} & \approx 1,908
\end{aligned}
$$
...
You just won the TVM Lottery. You will receive \$1 million today plus another 10 annual payments that increase by \$275,000 per year. Thus, in one year you receive \$1.275 million. In two years, you get \$1.55 million, and son on. If the appropriate interest rate is 9 percent, what is the present value of your winnings?

The growth rate is $g = .275$. This is greater than the interest rate of 9%. We cannot use the annuity formula. However, we can calculate the 10 annual payments manually.

The annuity is as follows:

$$
\begin{aligned}
\mathbf{PV} &= 1 + \frac{1.275}{(1 + .09)} + \frac{1.55}{(1 + .09)^2} + \cdots + \frac{1 + .275 \cdot n}{(1 + .09)^n} \\ \\
...
```{r problem 40}
sum((1 + .275 * (0:10))/(1 + .09)^(0:10))
```

The present value of the winnings is \$15,885,030.

---

## EAR versus APR

You have just purchased a new warehouse. To finance the purchase, you've arranged for a 30-year mortgage for 80 percent of the \$4,500,00 purchase price. The monthly payment on this loan will be \$27,500. What is the APR on this loan? The EAR?

---

$$
\begin{aligned}
\mathbf{PV}_{\text{loan}} &= .80 \cdot 4,500,000 \\
\mathbf{FV}_{\text{loan}} &= 0 \text{  (We are paying off the loan)} \\
\mathbf{PMT} &= 27,500 \\
\mathbf{m} &= 12 \\
\mathbf{n} &= m \cdot T = 12 \cdot 30 = 360
\end{aligned}
$$

$$
\begin{aligned}
\mathbf{PV}_{\text{loan}} &= \frac{27,500}{\left( 1 + \frac{r}{m}  \right)^{mT}} \\ \\
  3,600,000 &= \frac{27,500}{\left(1 + \frac{r}{12} \right)^{360}} \\ \\
\end{aligned}
$$
...
Consider a firm with a contract to sell an asset for \$115,000 three years from now. The asset costs \$76,000 to produce today. Given a relevant discount rate on this asset of 13 percent per year, will the firm make a profit on this asset? At what rate does the firm just break even?

To find if the firm will make a profit, calculate the present value of \$115,000 to find if it is greater than \$76,000.

$$
\begin{aligned}
\mathbf{PV} &= \frac{115,000}{(1 + .13)^3} \\ \\
 &= 79,7000.77
\end{aligned}
$$

So the firm will make a profit of \$3,700.77.

The break even interest rate is the rate where:

$$
\begin{aligned}
76,000 &= \frac{115,000}{(1 + r)^3}
\end{aligned}
$$
...
The break even interest rate is 14.8%
...
$$
\begin{aligned}
\mathbf{PV}_{\text{6 years from now}} &= \sum_{n=0}^{20}{\frac{5,000}{(1 + .06)^{n}}} \\ \\
  &= 5,000 \cdot \frac{1 - (1 + .06)^{-20}}{.06} \\ \\
  &= 57,349.61
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{PV}_{\text{today}} &= \frac{57,349.61}{(1 + .06)^{5}} \\ \\
  &= 42,854.96
\end{aligned}
$$
...
We have two separate annuities.

$$
\begin{aligned}
\mathbf{PV}_{\text{first}} &= 1,500 \cdot \frac{1 - (1 + .01)^{-84}}{.01} \\ \\
  &= 84,972.68 \\ \\
\mathbf{PV}_{\text{second}} &= 1,500 \cdot \frac{1 - (1 + .005)^{-96}}{.005} \\ \\
  &= 114,142.80
\end{aligned}
$$

The second calculation needs to be projected back to present day:

$$
\begin{aligned}
\mathbf{PV}_{\text{today}} &= \frac{114,142.80}{(1 + .01)^{84}} \\ \\
 &= 49,482.67
\end{aligned}
$$

Therefore the present value is the sum of the two:

$$
\begin{aligned}
  75,075.69 + 49,482.67 = 124,558.40
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{PV}_{\text{A}} &= 1,500 \times \frac{1 - (1 + \frac{.087}{12})^{180}}{\frac{.087}{12}} \\ \\
 &= 150,527.10
\end{aligned}
$$

For investment B to match this:

$$
\begin{aligned}
\mathbf{FV}_{\text{B}} &= C_0 \times e^{180} \\ \\
C_0 &= \frac{150,527.10}{e^{180}} \\ \\
 &= 45,337.89
\end{aligned}
$$
...
## Calculating present value of a perpetuity
...
So we can pretend that $t = 7$ is the present. Therefore, We find the present value of a perpetuity that begins $15 - 7 = 8$ years from now.
...
$$
\begin{aligned}
\mathbf{PV}_{\text{8 years previous}} &= \frac{40,983.61}{(1 + .061)^{7}} \\ \\
  &= 27077.12
\end{aligned}
$$
...
A local finance company quotes a 16 percent interest rate on one-year loans. So, if you borrow \$26,000, the interest for the year will be \$4,160. Because you must repay a total of \$30,160 in one year, the finance company requires you to pay \$30,160/12, or \$2,513.33, per month over the next 12 months. Is this a 15 percent loan? What rate would legally have to be quoted? What is the effective annual rate?

The loan involves equal monthly payments of \$2,513.33 for 12 months.

$$
\begin{aligned}
\mathbf{P} &= C \times \frac{1 - (1 + r)^{-n}}{n}
\end{aligned}
$$
...
The monthly interest is 2.36%.

The APR is: $2.36 \times 12 = 28.32\%$.

The EAR is: $(1 + r)^{m} - 1 = 32.31\%$

Thus, this is not a 15% loan it is significantly higher. The finance company would have to legally quote the __APR__ as 28.33%, reflecting the cost based on monthly compounding.

The __EAR__ of 32.31% accounts for the full compounding effect over the year.
...
\begin{aligned}
\mathbf{C_0} &= 5,300 \\
\mathbf{T} &= 2 \cdot 5 = 10 \\
\mathbf{r}_{\text{monthly}} &= \frac{r}{m} = \frac{.12}{12} = 0.01 \\
\mathbf{r}_{\text{semiannually}} &= (1 + \frac{r}{m})^{m} = (1 + \frac{.12}{12})^{6} - 1 = 0.0615
\end{aligned}
$$

$$
\begin{aligned}
\mathbf{PV}_{\text{annuity}} &= 5,300 \times \frac{1 - (1 + .0615)^{-10}}{.0615} \\
  &= 38,732.73
\end{aligned}
$$

Discount to 5 years from now

$$
\begin{aligned}
\mathbf{PV}_{\text{discounted five years from now}} &= \frac{38,732.73}{(1 + .01)^{48}} \\
  &= 24,024.38
\end{aligned}
$$

Discounted to 3 years from now

$$
\begin{aligned}
\mathbf{PV}_{\text{discount 3 years}} &= \frac{38,732.73}{(1 + .01)^{72}} \\
  &= 18,920.79
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{PV}_{\text{today}} &= \frac{38,732.73}{(1 = .01)^{108}} \\
 &= 13,224.21
\end{aligned}
$$
...
An ordinary annuity assumes that the first annuity payment begins a full period hence.

$$
\begin{aligned}
\mathbf{PV}_{\text{ordinary annuity}} &= C \times \left( \frac{1 - (1 + r)^{-T}}{r}  \right)
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{PV}_{\text{annuity due}} &= C + C \times \left( \frac{1 - (1 + r)^{-T}}{r}  \right)
\end{aligned}
$$

Which means that the first payment is at date 0. The annuity due has a higher present value than the ordinary annuity.

Thus the value of both annuity types:

$$
\begin{aligned}
\mathbf{PV}_{\text{ordinary annuity}} &= 20,000 \times \left( \frac{1 - (1 + .07)^{-5}}{.07}  \right) \\
  &= 82,003.95 \\
\mathbf{PV}_{\text{annuity due}} &= 20,000 + 20,000 \times \left( \frac{1 - (1 + .07)^{-5}}{.07}  \right)  \\
  &= 102,003.90
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{FV}_{\text{ordinary annuity}} &= C \times \frac{(1 + r)^n - 1}{r} \\ \\
\mathbf{FV}_{\text{annuity due}} &= \text{FV}_{\text{ordinary annuity}} \times (1 + r)
\end{aligned}
$$

$$
\begin{aligned}
\mathbf{FV}_{\text{ordinary annuity}} &= 20,000 \times \frac{(1 + .07)^5 - 1}{.07} \\
  &= 115,014.80 \\ \\
\mathbf{FV}_{\text{annuity due}} &= 115,014.80 \times (1 + .07) \\
  &= 123,065.80
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{r} &= \frac{.0645}{12} \\
  &= .0.005375  \\ \\
\mathbf{C} &= 73,000 \times \frac{.005375}{1 - (1 + .005375)^{-60}} \times (1 + .005375) \\
  &= 1434.29
\end{aligned}
$$
