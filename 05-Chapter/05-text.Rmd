...
* __capital budgeting__ The decision-making process for accepting or rejecting
  projects.

* __Net present value__ The value of future cash flows minus the present value
...
The basic investment rule for NPV can be genearlized to:

* Accept a project if the NPV is greater than zero.

* Reject a project if NPT is less than zero.
...
where $R$ is the discount rate. The IRR method is used to find $R$. We can use
`R`'s `uniroot` method to find this
...
* __independent project__ is a project whose acceptance or rejection is
  independent of the acceptance or rejection of other projects.

* __mutually exclusive investments__ When investing in one project means you
  cannot invest in the other project.
...
#### Problem 2: Mutliple rates or return
...
The problem with IRR is that it ignores issues of _scale_. Although Opportunity
