...
stipulated period. Liabilities are debts and are frequently associated witn
...
2. The nonoperating section of the income statement includes all financing
   acosts, such as interest expense.
...
* The matching principle of GAAP dictates that revenues be matched with
  ...
  nocash flow has necessarily occurred.
...
* __Deferred taxes__ result from differences between accounting income and true
  ...
  reporting purposes). The current tax is what is taxes paied. Deferred taxes is
  not. However, in theory, if taxable income is less than accounting income in
  the current year, it will be more than accounting income later on. Therefore,
  the taxes will have to be paid later on. The balance sheet will show deferred
  taxes as a liability.
...
* __Product costs__ are the total production costs incurred during a
  ...
  fixed costs are reproted in product costs.

* __Period costs__ are costs are costs that are allocated to a time period; they
...
```{r table 2.3: corporate tax rate, echo=F}
income <- c("&nbsp;18,333,335+          ", "15,000,001-18,333,333", "10,000,001-15,000,000",
            "    335,001-10,000,000", "    100,001-   335,000",
            "     75,001-   100,000", "     50,001-    75,000",
            "$         0-    50,000")
rate <- c("15%", "25 ", "34 ", "39 ", "34 ", "35 ", "38 ", "35 ")

df <- data.frame(rev(income), rate)
colnames(df) <- c("Taxable Income", "Tax Rate")
knitr::kable(df,
             align=c("r", "c"),
             caption="Corporate tax rates \\label{table:taxrates}")
```
...
* In addition to investing in fixed assets (i.e., capital spending), a firm can
  invest in net working capital. This is called the __cahnge in net working
...
```{r changes in fixed assets, echo=F}
m <- c(198, -25)
df <- data.frame(c(m, sum(m)))
colnames(df) <- c("$ in millions")
...
```{r total cash flow of the firm, echo=F}
m <- c(238, -173, -23)
df <- data.frame(c(m, sum(m)))
colnames(df) <- c("$ in millions")
...
```{r cash flow paid to creditors, echo=F}
m <- c(49, 73)
m.1 <- c(m, sum(m))
m.2 <- c(m.1, -86)
...
```{r cash flow paid to stockholders, echo=F}
m <- c(43, 6)
m.1 <- c(m, sum(m))
m.2 <- c(m.1, -43)
df <- data.frame(c(m.2, sum(m.2)))
...
```{r cash flow from operating activities, echo=F}
...
```{r cash flow from investing activities, echo=F}
m <- c(-198, 25)
...
```{r cash flow from financing activities, echo=F}
m <- c(-73, 86, -3, -43, -6, 43)
...
From Table \ref{table:taxrates}, we see taht the tax rate appled to the first \$50,000
...
__True__. All assets are liquid at some price. However, the degree of liquidity
and the ability to sell an asset varies. Assets are usually classified based on
its liquidity. _Highly liquid assets_ are assets that can be converted to cash
very quickly and with minimal loss to value. _Less liquid assets_ cannot be
easily converted to cash without significant loss to value.
...
There are several reasons why a standard income statement does not represent the
actual inflows and outflows that occur during the accounting period.

### Accrual accounting---U.S. GAAP accounting

Income statements are typically prepared using the _accrual basis of
accounting_, which recognizes revenue when it is earned and expenses when they
are incurred, regardless of when cash is actually received or paid.

### Non-cash expenses

Income statements include non-cash expenses such as _deprecation_, _deferred
taxes_ and _amortization_. These items reduce reported profits but do not
include any cash outflows.

### Financing activities excluded

Cash flows related to financing activities, such as borrowing money, issuing
stock, or repaying debt, do not appear on the income statement but do impact
actual cash flow.

### Capital expenditures omitted

The purchase of long-term assets, called _capital expenditures_, represents
significant cash outflows but is not recorded as an expense on the income
statement. Instead, the cost is allocated over time through depreciation.

### Changes in working capital

Changes in current assets and liabilities, such as accounts receivable, accounts
payable, and inventory, affect cash flows but are not directly captured on the
income statement.

### Summary

The income statement provides a measure of profitability based on accounting
principles, but actual cash inflows and outflows are better captured in the
_cash flow statement_, which reconciles net income with cash changes by
adjusting for non-cash items, timing differences, and financing/capital
activities.
...
issues (e.g., excessive capital spending, operaitonal inefficiencies).
...
1. __Limited Liability:__ Shareholders cannot owe more than the value of their
   equity investment. The worst-case scenario is that equity becomes worhtless.
...
A negative net working capital means that the current liabilities have
...
- By company collecting accounts receivable faster, reducing outstanding
  balances.
...
case of Bank of America's \$34 billion write-off for bad loands, the company
...
that the company is taking aciton to acknowledge and deal with problems early.
...
Calculate the current assets of Klingon Cruisers Inc:
...
Now calculate the book value of all assets:
...
Calculate the market value of all assets.
...
following Table, calculate the company's income taxes. What is the average tax
rate? What is the marginal tax rate?

```{r problem 4: calculating taxes: corporate tax table, echo=F}
...
\text{Taxes owed} &= 7,500 + 6,250 + 8,500 + 67,470\\
  &= 89,720\\ \\
\text{Average Tax rate} &= \frac{89,720}{273,000}\\
  &= 0.33\\ \\
\text{Marginal Tax rate} &= .039
\end{aligned}
$$

#### We can calcualte the solution in `R`

The following functions will use the tax brackets from the tax rates table to calculate income taxes and find the marginal tax rate.
...
Using these functions we can calculate the income tax, the average tax rate and the marginal tax rate.
...
Given:
  ...
  * Net fixed assets: (2011) \$1,420,000, (2012) \$1,690,000
  * Depreciation: (2012) \$145,000
...
capital.surplus <- 45
tb <- matrix(c(long.term.debt, preferred.stock, common.stock.1.par.value,
               accumulated.retained.earnings, capital.surplus), ncol=1)
