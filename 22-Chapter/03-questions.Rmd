...
p1$E.a = 60
p1$E.b = 50
p1$V.a = p1$S0 - p1$E.a/(1 + p1$r)
p1$V.b = p1$S0 - p1$E.b/(1 + p1$r)
```

## A. Value of call option with a \$60 exercise price


The intrinsic value of the stock is \(\$63 - 60 = \$3\). Since the stock price
has a floor of \$61 for one year, there is no way the option is worthless in one
year. This means that the put option is worthless. We should not use the
risk-neutral model to calculate the value of the call, we should use the
put-call parity model with the put value = 0.

\[
\begin{aligned}
\text{Call option value} = 63 - \frac{60}{1.048}
  = \$`r round(p1$V.a, 2)`
\end{aligned}
\]
...
This is even lower than for part (a). The intrinsic valueof the stock is \(\$63
- 50 = \$13\). The stock still has a floor above the exercise price of \$50.
  This means we should use the put-call parity model:

\[
\begin{aligned}
\text{Call option value} = 63 - \frac{50}{1.048}
  = \$`r round(p1$V.b, 2)`
\end{aligned}
\]
...
Since the put option exercise price of \$60 is below the floor stock price of
\$61, then the put option will always be worthless.
...
The maximum gain of 10 August 110 put options occur if the stock price goes to
...
When selling put options, I take on the risk (the obligation) of buying the
stock at the exercise price on the expiration date. If the stock price is \$103
then I would incur a loss of \(4.70 \times 10 \times 100 - (110 - 103) \times
1,000 = \$7,000 = -\$2,300\).

If the stock price is \$132 on expiration then the option is out of the money
and is worth zero. I keep the \$4,700 premium.
...
p4$p.a = uniroot(function(p) p*96 + (1 - p)*74 - 80*1.05, c(0,1))$root
p4$V = (p4$p.a * p4$C.swing.a[2] + (1 - p4$p.a)*p4$C.swing.a[1])/(1 + p4$r)
```
...
The expected stock price under risk-neutral measure must be the forward price:

\[
\begin{aligned}
80 \times 1.05 &= p^* \cdot 96 + (1 - p^*) \cdot 74\\
84 &= 96p^* + 74 - 74p^*\\
p &= \frac{10}{22} \approx `r round(p4$p.a, 2)`
\end{aligned}
\]
...
\[
\begin{aligned}
C &= \frac{p^* \cdot 26 + (1 - p^*) \cdot 4}{1.05}\\
  &= \frac{0.45 \times 26 + 0.54 \times 4}{1.05}\\
  &= \frac{11.82 + 2.18}{1.05} = \frac{14}{1.05} \approx `r round(p4$V, 2)`
\end{aligned}
\]
...
\[
S_0 \times (1 + r) = p^* \times S_u + (1 - p^*) \times S_d
  = `r round(uniroot(function(p) p*96 + (1-p)*74 - 80*1.05, c(0,1))$root, 2)`
\]

\[
V = \frac{p^* \times \max(S_u - E, 0) + (1 - p^*) \times \max(S_d - E, 0)}
    {1 + r} = \$`r round((.45 * 6 + 0)/(1.05),2)`
\]

Book answer: \$13.33, \$2.60
\[
\frac{2.60 - 2.55}{2.60} = `r round((2.6-2.55)/2.6,2)*100`\%
\]

\[
\frac{2.60 - 2.57}{2.60} = `r round((2.6 - 2.57)/2.6, 2)*100`\%
\]
...
```{r problem 5, echo=F}
p5 = new.env()
p5$prob = round(uniroot(function(p) p*70+(1-p)*50 - 62*1.05, c(0,1))$root, 2)
```

## A. Current price \$62, Exercise price of \$35

We use the put-call parity model with the put value = 0. This is because a put
option with an exercise price of \$35 is worthless if the minimum price the
stock will be is \$50.

\[
V = S_0 - \frac{E}{1 + r} = 62 - \frac{35}{1.05}
  = \$`r round(62 - 35/1.05, 2)`
\]
...
\[
\text{Borrow} = \frac{25}{1.05} = \$`r round(25/1.05, 2)`
\]
...
\[
62 \times (1.05) = p^* \times 70 + (1 - p^*) \times 50
\]

\[
V = \frac{p \times \max(S_u - E, 0) + (1 - p) \times \max(S_d - E, 0)}{1+r}
  = \$`r round((p5$prob*10 + (1 + p5$prob)*0)/1.05, 2)`
\]
...
A put option and a call option with an exercise price of \$55 expire in two
months and sell for \$2.65 and \$5.32, respectively. If the stock is currently
priced at \$57.30, what is the annual continuously compounded rate of interest?
...
The Delta shows how the change in the stock price affects the value of the call
or put. For call options, the stock price and the value is correlated. For put
options they are inversely correlated.
...
p16$T = 1
p16$swing = .15
p16$p = uniroot(function(p)
    p * p16$swing + (1 - p)*(-p16$swing) - p16$r, c(0,1))$root
p16$C = (p16$p * (p16$S * (1 + p16$swing)-p16$E))/(1 + p16$r)
```

The Risk-neutral valuation calculates the probabilities of the two states by
assuming that the return equals the risk free rate. We can calculate this
return.

\[
`r p16$r` = p^* \times `r p16$swing` + (1 - p^*) \times (`r -p16$swing`)
\]

We solve for \(p^* = .7667\).

Then the value of the call is calculated:

\[
\text{Value of call} = \frac{.7667 \times (84 - 70) + (1 - .7667) \times 0}
                            {1.08} = \$`r p16$C`
\]
...
p17 = new.env()
p17$r = .05
p17$p = uniroot(function(p)
    p * p16$swing + (1 - p) * (-p16$swing) - p17$r, c(0, 1))$root
p17$C = (p17$p * (p16$S * (1 + p16$swing) - p16$E))/(1 + p17$r)
```

We calculate the new probability:

\[
`r p17$r*100`\% = \text{Probability of a rise} \times `r p16$swing*100`\%
   + (1 - \text{Probability of rise}) \times -`r p16$swing*100`\%
\]
...
\text{Value of call} = \frac{`r pretty(p16$p, 3)`
    \times \$`r pretty(p16$S * (1 + p16$swing))`
    + (1 - `r pretty(p17$p, 3)`) \times 0)}{(`r 1 + p17$r`)}
    = `r pretty(p17$C)`
\]


The risk-neutral probability of a stock increase decreases as the risk-free rate
decreases.
...
p28$C.swing = C.swing(p28$S.swing[1], p28$S.swing[2], p28$E)
p28$delta = delta(p28$S.swing[1], p28$S.swing[2], p28$E)
p28$borrow.payback = borrow.payback(p28$S.swing[1], p28$S.swing[2], p28$E)
p28$borrow = borrow(p28$S.swing[1], p28$S.swing[2], p28$E, p28$r.ear, p28$T)
p28$C.ts = C.twostate(p28$S, p28$S.swing[1], p28$S.swing[2],
                      p28$E, p28$r.ear, p28$T)
p28$p = uniroot(function(p){
        p * (1605/1450 - 1) + (1 - p)*(1340/1450 - 1) - p28$r
    }, c(0, 1))$root
p28$C.p = (p28$p * p28$C.swing[2] + (1 - p28$p) * p28$C.swing[1]) /
          (1 + p28$r)
```
...
\text{Value of the call} &= \frac{p \times \text{Stock high}
    + (1 - p) \times \text{Stock low}}{(1 + r_{\text{qtr}})}\\
    &= \frac{`r pretty(p28$p, 4)` \times `r p28$C.swing[2]`
    + (1 - `r pretty(p28$p, 4)`) \times `r p28$C.swing[1]`}
      {(1 + `r pretty(p28$r, 4)`)}\\
    &= \$`r pretty(p28$C.p)`
\end{aligned}
\]
...
Maverick can buy gold and borrow at the present value of the exercise price.

The delta is

\[
\text{Delta} = \frac{\text{Swing of call}}{\text{Swing of gold}}
    = \frac{`r pretty(p28$C.swing[2], 0)` - `r pretty(p28$C.swing[1], 0)`}
           {`r pretty(p28$S.swing[2], 0)` - `r pretty(p28$S.swing[1], 0)`}\\
    = `r pretty(p28$delta, 4)`
\]

We need to borrow

\[
\text{Borrow} = `r pretty(p28$borrow.payback)`
\]

## D. The synthetic call is the same as the real option
